...
function getMapOverlayCopy(context, primary) {
  if (context.zoneHint && primary.zone === context.zoneHint) {
    return {
      title: "Vista pública del mapa",
      detail: `Ya ves la zona que mejor encaja ahora mismo: ${primary.zoneLabel}. El mapa completo y el detalle fino quedan bloqueados.`
    };
  }

  if (isLateArrival(context.hour)) {
    return {
      title: "Vista pública del mapa",
      detail: `Ya ves una base con encaje rápido para esta hora, pero no el mapa completo ni el contraste fino entre zonas.`
    };
  }

  return {
    title: "Vista pública del mapa",
    detail: "Ya ves posiciones aproximadas y precio base, pero no la lectura completa del mapa ni el encaje fino por zona."
  };
}

function buildMapPins(ranked, primary) {
  return ranked.slice(0, 5).map((option, index) => ({
    id: option.id,
    label: option.label,
    zoneLabel: option.zoneLabel || option.label,
    priceText: formatPrice(option.baseNightEur),
    x: option.mapPosition?.x ?? 50,
    y: option.mapPosition?.y ?? 50,
    isPrimary: option.id === primary.id,
    visibility: index === 0 ? "high" : "limited"
  }));
}
...
function formatAlternative(option) {
  return {
    ...
    type: option.type,
    priceText: formatPrice(option.baseNightEur),
    reasonLine: getReasonLine(option)
  };
}
...
export function getContextualLodgingRecommendation({ arrivalType, arrivalData, transportMode }) {
  ...
  const alternatives = ranked.slice(1, 3).map(formatAlternative);
  const overlayCopy = getMapOverlayCopy(context, primary);
  ...
  return {
    ...
    primary: {
      ...
      label: primary.label,
      zone: primary.zone,
      zoneLabel: primary.zoneLabel || primary.label,
      type: primary.type,
      priceText: formatPrice(primary.baseNightEur),
      reasonLine: getReasonLine(primary),
      caution: primary.caution,
      publicNote: primary.publicNote,
      areaHint: primary.areaHint,
      coordinates: primary.coordinates,
      mapPosition: primary.mapPosition
    },
    alternatives,
    alternativesText:
      alternatives.length > 0
        ? alternatives.map((option) => `${option.label} desde ${option.priceText}/noche`).join(" · ")
        : "",
    mapPreview: {
      title: overlayCopy.title,
      detail: overlayCopy.detail,
      pins: buildMapPins(ranked, primary),
      lockedMessage: "La posición exacta, el detalle completo y la lectura fina de zona quedan en Pro."
    },
    meta: {
      arrivalReference: context.reference,
      load: context.load,
      hour: context.hour,
      zoneHint: context.zoneHint,
      lateArrival: isLateArrival(context.hour)
    }
  };
}
